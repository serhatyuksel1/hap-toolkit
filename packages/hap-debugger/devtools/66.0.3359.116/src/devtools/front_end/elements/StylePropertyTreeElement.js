...
Elements.StylePropertyTreeElement = class extends UI.TreeElement {
  ...
  async _innerApplyStyleText(styleText, majorChange) {
    ...
    // hotfix: 根据插入css property的情况，调整得到的CSSProperty对象的索引
    var liElementList = this.treeOutline.element.querySelectorAll('li')
    var alternativeIdx
    // 当插入到StyleSidebarSection中的li元素个数超过已通过CDP命令设定好的properties个数。这包括在最后追加，在中间插入等情况;
    if (liElementList.length > this._style.allProperties.length) {
      for (var k=0; k<this._style.allProperties.length; k++) {
        var propName = this._style.allProperties[k].name
        var editingName = this.property.name
        if (propName === editingName) {
          alternativeIdx = k
          break
        }
      }

      // 对于不支持的属性，直接删除; e.g. listItem上的postion:fixed
      if (typeof alternativeIdx === 'undefined') {
        var notMatchedLi = liElementList[this.property.index]
        notMatchedLi.parentElement.removeChild(notMatchedLi)
      }
    }

    this.property = (this._style.propertyAt(typeof alternativeIdx == 'number' ? alternativeIdx : this.property.index)) || this.property;;

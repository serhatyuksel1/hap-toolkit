...
SDK.CSSRule = class {
  ...
  constructor(cssModel, payload) {
    ...
    if (this.styleSheetId) {
      ...
      this.sourceURL = styleSheetHeader && styleSheetHeader.sourceURL;
...
SDK.CSSStyleRule = class extends SDK.CSSRule {
  ...
  selectorRange() {
    // hotfix: Adapt to the previous version
    const firstRange = this.selectors[0] ? this.selectors[0].range : null;
    if (!firstRange)
      return null;
  ...
  rebase(edit) {
    ...
    if (this.selectorRange() && this.selectorRange().equal(edit.oldRange)) {
    // hotfix: end
